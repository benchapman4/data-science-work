...
![](c00-diamonds-assignment-BEN_files/figure-gfm/q1-task-1.png)<!-- -->

**Observations**:

  - (Write your observations here\!) Larger carat tends to correspond to
    larger price. Smaller carat has a smaller variability in price than
    larger carat. Carat appears to be somewhat noncontinuous, with
    certain buckets that have much higher counts.
...
ggplot(data = diamonds) +
  geom_point( mapping=aes(x = carat, y = price, color = cut), alpha = .8)
```

![](c00-diamonds-assignment-BEN_files/figure-gfm/q2-task-1.png)<!-- -->

**Observations**:

At a given size, the ideal tend to have a highest price and fair tends
to have lowest price. However at a given size there is price variation
that cannot be fully explained by cut.
